...
const resolveIcon = (iconName) => {
  return markRaw(Icons[iconName] || Icons.GridIcon)
}
...
const isSubmenuOpen = (groupIndex, itemIndex) => {
  ...
  if (!groups[groupIndex]) return false
  if (!groups[groupIndex].items[itemIndex]) return false
...
const fetchMenus = async () => {
  ...
  const { success, data, error } = await Menu
    .orderBy('group', 'asc')
    .orderBy('order', 'asc')
    .get()
  ...
  const raw = data.map(m => m.attributes)

  // Pisahkan parent & child
  const parents = raw.filter(m => !m.parent_id)
  const children = raw.filter(m => m.parent_id)

  // Grouping by "group"
  const grouped = {}

  parents.forEach(parent => {
    if (!grouped[parent.group]) {
    ...
    grouped[parent.group].items.push({
      name: parent.name,
      icon: resolveIcon(parent.icon),
      path: subItems.length ? null : parent.path,
      subItems: subItems.length ? subItems : null
    })
  })

  menuGroups.value = Object.values(grouped)
}
